...
var (
	outfile, _ = os.Create("/tmp/img-cache.log")
	l = log.New(outfile, "", 0)
)

func NewInMemoryCacheStorage() solver.CacheKeyStorage {
...
type inMemoryKey struct {
	id        string
	results   map[string]solver.CacheResult
	links     map[solver.CacheInfoLink]map[string]struct{}
	backlinks map[string]struct{}
}

func (s *inMemoryStore) Exists(id string) bool {
	l.Printf("Checking Exists: %s\n", id)
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *inMemoryStore) WalkResults(id string, fn func(solver.CacheResult) error) error {
	s.mu.RLock()
	...
	copy := make([]solver.CacheResult, 0, len(k.results))
...
func (s *inMemoryStore) Load(id string, resultID string) (solver.CacheResult, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	k, ok := s.byID[id]
	if !ok {
		return solver.CacheResult{}, errors.Wrapf(solver.ErrNotFound, "no such key %s", id)
	}
...
func (s *inMemoryStore) AddResult(id string, res solver.CacheResult) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	k, ok := s.byID[id]
	if !ok {
...
func (s *inMemoryStore) AddLink(id string, link solver.CacheInfoLink, target string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	k, ok := s.byID[id]
	if !ok {
...
func (s *inMemoryStore) WalkLinks(id string, link solver.CacheInfoLink, fn func(id string) error) error {
	s.mu.RLock()
	k, ok := s.byID[id]
	if !ok {
		s.mu.RUnlock()
...
func (s *inMemoryStore) HasLink(id string, link solver.CacheInfoLink, target string) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if k, ok := s.byID[id]; ok {
...
func (s *inMemoryStore) WalkBacklinks(id string, fn func(id string, link solver.CacheInfoLink) error) error {
	s.mu.RLock()
	k, ok := s.byID[id]
	if !ok {
		s.mu.RUnlock()
	...
	for bid := range k.backlinks {
		...
		for l, m := range b.links {
			...
			outLinks = append(outLinks, solver.CacheInfoLink{
...
func NewInMemoryResultStorage() solver.CacheResultStorage {
...
func (s *inMemoryResultStore) Save(r solver.Result) (solver.CacheResult, error) {
	l.Printf("Saving ID: %s\n", r.ID())
	s.m.Store(r.ID(), r)
	return solver.CacheResult{ID: r.ID(), CreatedAt: time.Now()}, nil
}

func (s *inMemoryResultStore) Load(ctx context.Context, res solver.CacheResult) (solver.Result, error) {
	v, ok := s.m.Load(res.ID)
	if !ok {
		return nil, errors.WithStack(solver.ErrNotFound)
	}
	return v.(solver.Result), nil
}

func (s *inMemoryResultStore) LoadRemote(ctx context.Context, res solver.CacheResult) (*solver.Remote, error) {
	return nil, nil
}
